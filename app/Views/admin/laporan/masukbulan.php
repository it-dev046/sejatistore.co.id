<?= $this->extend('admin/layout/template')  ?>
<?= $this->Section('content');  ?>
<div id="layoutSidenav_content">
    <main>
        <div class="container-fluid px-4">
            <h1 class="mt-4"> <?= $title; ?></h1>
            <ol class="breadcrumb mb-4">
                <li class="breadcrumb-item"><a href=" <?= base_url('dashboard-admin') ?> ">Dashboard</a></li>
                <li class="breadcrumb-item active"> <?= $title; ?></li>
            </ol>
            <div class="card mb-4">
                <div class="card-body">
                    <div class="card mb-4">
                        <div class="card-header">
                            <i class="fas fa-table me-1"></i>
                            Daftar Pemasukan Bulan ini
                        </div>

                        <div class="card-body">
                            <!-- Notifikasi Berhasil -->
                            <?php if (session('success')) : ?>
                                <div class="alert alert-success" role="alert">
                                    <?= session('success'); ?>
                                </div>
                            <?php endif; ?>
                            <table id="table" class="table table-striped table-bordered">
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>No Faktur / Invoice</th>
                                        <th>Tanggal</th>
                                        <th>Pelanggan</th>
                                        <th>Transaksi</th>
                                        <th>Total Bayar</th>
                                        <th>Keterangan</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php $no = 1; ?>
                                    <?php foreach ($daftar_bayar as $transkasi) : ?>
                                        <!-- html... -->
                                        <?php if ($transkasi['id_trans'] == 0) { ?>
                                        <?php } else { ?>
                                            <tr>
                                                <td> <?= $no++; ?> </td>
                                                <td>
                                                    <?= $transkasi['id_trans']; ?>
                                                </td>
                                                <td> <?= date('d M Y', strtotime($transkasi['tanggal'])); ?></td>
                                                <td> <?= $transkasi['nama_pel']; ?> (<?= $transkasi['nama_katepel']; ?>) </td>
                                                <?php if ($transkasi['totalbayar'] == 0) { ?>
                                                    <td>0</td>
                                                <?php } else { ?>
                                                    <td><?= number_to_currency($transkasi['totalbayar'], 'IDR', 'id_ID') ?></td>
                                                <?php } ?>
                                                <?php if ($transkasi['total'] == 0) { ?>
                                                    <td><?= $transkasi['total']; ?></td>
                                                <?php } else { ?>
                                                    <td><?= number_to_currency($transkasi['total'], 'IDR', 'id_ID') ?></td>
                                                <?php } ?>
                                                <td> <?= $transkasi['keterangan']; ?></td>
                                            </tr>
                                        <?php } ?>
                                    <?php endforeach; ?>
                                    <?php foreach ($daftar_bayarpasang as $value) : ?>
                                        <!-- html... -->
                                        <?php if ($value->invoice == 0) { ?>
                                        <?php } else { ?>
                                            <tr>
                                                <td> <?= $no++; ?> </td>
                                                <td>
                                                    <?= $value->invoice; ?>
                                                </td>
                                                <td> <?= date('d M Y', strtotime($value->tanggal)); ?></td>
                                                <td> <?= $value->nama; ?></td>
                                                <?php if ($value->biaya == 0) { ?>
                                                    <td>0</td>
                                                <?php } else { ?>
                                                    <td><?= number_to_currency($value->biaya, 'IDR', 'id_ID') ?></td>
                                                <?php } ?>
                                                <?php if ($value->bayar == 0) { ?>
                                                    <td>0</td>
                                                <?php } else { ?>
                                                    <td><?= number_to_currency($value->bayar, 'IDR', 'id_ID') ?></td>
                                                <?php } ?>
                                                <td> <?= $value->keterangan; ?></td>
                                            </tr>
                                        <?php } ?>
                                    <?php endforeach; ?>
                                </tbody>
                                <tfoot>
                                    <tr>
                                        <th colspan="5" class="text text-center">
                                            Total Pendapatan Bulan ini
                                        </th>
                                        <th>
                                            <?php $pemasukanbulan = $totalbayarbulan + $masukbulanan; ?>
                                            <?php if ($pemasukanbulan > 0) { ?>
                                                <?= number_to_currency($pemasukanbulan, 'IDR', 'id_ID',) ?>
                                            <?php } else { ?>
                                                Rp 0
                                            <?php } ?>
                                        </th>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </main>
    <?= $this->endSection() ?>
    <?= $this->Section('script') ?>
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('#table').DataTable({
                buttons: ['copy', 'csv', 'print', 'excel', 'pdf', 'colvis'],
                dom: "<'row'<'col-md-3'l><'col-md-5'B><'col-md-4'f>>" +
                    "<'row'<'col-md-12'tr>>" +
                    "<'row'<'col-md-5'i><'col-md-7'p>>",
                lengthMenu: [
                    [5, 10, 25, 50, 100, -1],
                    [5, 10, 25, 50, 100, "All"]
                ]
            });

            table.buttons().container()
                .appendTo('#table_wrapper .col-md-5:eq(0)');
        });
    </script>
    <?= $this->endSection() ?>